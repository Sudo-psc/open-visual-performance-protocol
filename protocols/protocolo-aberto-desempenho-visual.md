...
> O livro torna visível o custo invisível. O OVPP transforma essa tese em um sistema operacional de ação.

## 0) Papel como produto

O OVPP é a camada aberta de implementação ao lado do livro.
...
- tratar carga visual como variável operacional,
- reduzir fricção visual evitável com intervenções de baixo risco,
...
## 2) Usuários e contexto de compra
...
Melhores primeiros times:

- engenharia de software e revisão de código,
- QA e validação de dados,
- suporte ou sucesso do cliente com alta carga de vídeo/chat,
- financeiro, jurídico, compliance e análise,
...
- **Carga visual:** demanda sustentada imposta ao sistema visual por telas, foco próximo, reuniões, múltiplos monitores, iluminação, fluxo de ar e intensidade cognitiva.
- **Fricção visual:** perda evitável de conforto, estabilidade ou resistencia ao ler, focar, revisar, programar, desenhar ou comúnicar por telas.
- **Presenteísmo visual:** perda de produtividade enquanto a pessoa continua presente e trabalhando, frequentemente por releitura, revisão mais lenta, desconforto, evitação, pausas não planejadas ou retrabalho.
- **Padrão operacional:** sinal agregado por função, time, local, modalidade ou período.
- **Caso clínico:** situação individual de saúde que exige avaliação licenciada.

O protocolo se apoia em uma fronteira: padrões operacionais pertencem a organização; diagnóstico individual pertence a clínica.

## 5) Pacote mínimo viável do OVPP

Um piloto sério precisa destes artefatos antes do lançamento:

- declaração de propósito em linguagem simples,
- instrumento de triagem escolhido e datas fixas de medição,
- definição de grupos de função e exposição,
- checklist ambiental,
- playbook de pausas e fluxo de trabalho,
...
| Saúde ocupacional | trilha de risco, escalonamento e alinhamento de política | substituir diagnóstico oftalmológico |
...
### Fase 0 - Acordo de patrocínio

Definir grupo-alvo, dono, orçamento, datas, fronteira de privacidade e critérios de sucesso.
...
- linha de base em D+0,
- linha intermediária opcional em D+45 ou D+60,
- reavaliação final em D+90,
- mesmas perguntas e mesma lógica de pontuação durante o ciclo,
- coletar apenas identificadores agregáveis como grupo de função, área, modalidade e categoria ampla de exposição.
...
- função ou papel,
- horas de tela e carga de reuniões,
- blocos de foco profundo,
- uso de múltiplos monitores ou notebook isolado,
- trabalho remoto/presencial/híbrido,
- restrições ambientais,
- tarefas visuais de alta precisão como QA, revisão, análise ou design.

Saídas:

- mapa de carga visual por função,
- decisão de coorte prioritária,
- lista de hipóteses ambientais e de fluxo,
- lista de atenção para encaminhamento tratada apenas pela trilha de saúde apropriada.

### Camada 2 - Padronização ambiental
...
- não priorizar filtros de luz azul como principal investimento preventivo sem novo pacote robusto de evidência.

### Camada 3 - Pausas como infraestrutura

Tratar recuperação visual como desenho de fluxo, não como força de vontade.

Cadência recomendada:

- a cada 20 minutos: 20 segundos de foco distante e reset de piscadas,
- a cada 90 minutos: 5-10 minutos com menor demanda de foco próximo,
- reuniões longas: um segmento sem tela ou longe da tela quando possível,
- trabalho de alta precisão: pausas protegidas antes de revisão, QA, análise ou decisão prolongada.

Opções de implementação:

- padrões de calendário,
- lembretes em IDE ou navegador,
- prompts em Slack/Teams,
- templates de reunião,
- rituais de time,
- ferramentas offline que não rastreiem produtividade.

Sucesso não é "as pessoas foram avisadas para pausar". Sucesso é presença sustentada do prompt, adesão e reavaliação de sintomas.
...
| Grupo | Padrão | Ação |
|---|---|---|
| Baixo | sintomas leves e raros; sem impacto funcional | educação, checklist ambiental, próxima triagem |
| Moderado | sintomas semanais; impacto funcional; associação com tela/ambiente | correção ambiental, pausas estruturadas, reavaliação em 60-90 dias |
| Alto | sintomas persistentes, embaçamento recorrente, desconforto forte, piora com lentes, uso frequente de colírio sem orientação | trilha de saúde ocupacional e encaminhamento oftalmológico |

Critérios de encaminhamento imediato:

- dor ocular importante,
- piora relevante da visão,
- sintomas unilaterais marcantes,
- secreção intensa,
- suspeita de infecção ou inflamação,
- trauma,
- fotofobia súbita importante,
- piora em usuário de lente de contato.

Pacote de encaminhamento, com consentimento:

- função e padrão de exposição,
- horas estimadas de tela e chamadas,
- restrições de home office ou posto,
- faixa de escore e padrão de sintomas,
- intervenções ambientais já executadas.

### Camada 5 - Reavaliação e governança

Em D+90, comparar linha de base e medição final usando o mesmo instrumento e a mesma lógica de agregação.
...
| Aderência a pausas | autodeclaração ou sinal não invasivo | mensal |
| Volume de encaminhamentos por faixa | agregado da saúde ocupacional | trimestral |
| Foco e qualidade percebidos | mini-enquete interna | trimestral |
...
- nenhum resultado individual para gestores,
- nenhuma participação obrigatória,
- nenhum diagnóstico por equipe não clínica,
- dados de saúde separados de avaliação de performance,
- encaminhamento por canais de saúde com consentimento,
- política clara de retenção,
- aviso aos participantes em linguagem simples.

Promessa mínima ao participante:

> Este programa mede carga visual agregada e fricção do trabalho. Ele não é avaliação de performance, e respostas individuais sobre sintomas não serão compartilhadas com gestores.

## 10) Embalagem de produto como complemento do livro

Escada recomendada:

1. **Livro:** cria consciência e urgência executiva.
...
O protocolo aberto deve continuar utilizável sem compra adicional. Essa abertura aumenta confiança e facilita justificar implementação paga, parceria clínica ou advisory.

## 11) Definição de concluído para um piloto forte

Um piloto só está completo quando:
...
- [ ] Definir grupos de função, coorte alvo e dono do piloto.
- [ ] Publicar promessas de privacidade e não uso.
- [ ] Rodar linha de base e gerar mapa agregado.
- [ ] Executar padronização ambiental.
...
O OVPP é aberto e deliberadamente conservador. Ajuste limiares aos requisitos legais e clínicos locais, mas preserve sequência, fronteira de privacidade e postura anti-hipérbole. É isso que o torna um complemento crível ao livro, não apenas uma campanha de bem-estar com marca melhor.
